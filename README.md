...
**Core Game Engine:**
- **`card.go`**, **`deck.go`**, **`player.go`** - Game entity models with value comparisons
- **`game.go`** - State management (game flow, win conditions, war scenarios)

**Presentation Layer:**
- **`main.go`** - UI orchestration and Fyne window management
- **`effects.go`** - Visual effects (shake animations, card flips)
- **`sound.go`** - Audio system with embedded MP3 handling

**Asset Pipeline:**
- **`resource_helper.go`** - Maps bundled resources to file paths
- **`resources.go`** - Auto-generated by `fyne bundle` (6.7MB embedded assets)
- **`bundle_assets.sh`** - Automation script for asset bundling

This architecture made adding features easy - want new card effects? Add to `effects.go`. New sound? Just `sound.go`.
...
**CGO Dependencies**: Didn't expect GUI apps to require C compilers. Learning Zig for cross-compilation was an unexpected detour.

**Asset Size**: Card images are HUGE. The executable went from 10MB to 34MB after bundling. Learned about resource optimization the hard way.

**UI Threading**: Coming from Python, Go's explicit goroutine management for UI updates was initially confusing but makes so much sense now.

**Mac Packaging**: Creating a `.dmg` required learning about `.app` bundles, code signing, and macOS's security model. More complex than Windows `.exe`!
